...
class DualArmTeleopSystem:
        ...
        with self._kb_lock:
            if self._key_pressed['s']:
                ...
                if self._record_mode:
                    print("\n✅ 记录模式已开启")
                    self._episode_data = []  # 清空之前的数据
                    print(f"   记录目录: {self._record_directory}")
                else:
                    print("\n⏸️  记录模式已关闭")
                    if self._episode_data:
                        print(f"   当前已记录 {len(self._episode_data)} 步，按 'q' 保存")
                self._key_pressed['s'] = False  # 重置标志
    ...
    def run(self):
        ...
        print("  [Space] 按住:        断开模式 (Hold)")
        print("  [s] 开始/停止记录模式")
        print("  [q] 完成一次记录，保存 HDF5 文件")
        print("  [esc] 退出程序")
        if self.env._prompt:
