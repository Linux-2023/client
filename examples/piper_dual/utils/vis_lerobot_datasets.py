...
def print_episode_info(dataset: LeRobotDataset, episode_index: int):
    ...
    # 获取该episode的帧索引范围
    # 旧版本 lerobot 使用 episode_data_index
...
def print_all_episodes_tasks(dataset: LeRobotDataset):
    ...
    # 使用旧版 lerobot 的 episode_data_index
    task_counts = {}
...
def main():
    ...
    parser.add_argument(
        "--repo-id",
        type=str,
        default="my_piper_dual_dataset",
        help="LeRobot数据集的repo_id（如 my_piper_dual_dataset）"
    )
    parser.add_argument(
        "--root",
        type=str,
        default="./datasets/dual",
        help="LeRobot数据集的本地根目录（默认: ./datasets/dual）"
    )
    parser.add_argument(
        "--episode",
        type=int,
    ...
    # 加载数据集
    # 旧版 lerobot 使用 repo_id 作为本地路径
    print(f"正在加载数据集: repo_id={args.repo_id}, root={args.root}")
    try:
