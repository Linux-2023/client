#!/usr/bin/env python3
"""ROS 2-only dual-arm Piper EEF policy deployment entrypoint."""
...
@dataclass
class Args:
    out_dir: Path = Path("data/piper_dual_eef/videos")
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Deploy a dual-arm Piper EEF policy through ROS 2 PosCmd control.",
...
def _validate_eef_contract(args: Args) -> None:
    if args.publish_actions and args.dry_run:
        raise ValueError("--publish-actions requires --no-dry-run")
    if args.max_action_delta is not None and (not math.isfinite(args.max_action_delta) or args.max_action_delta < 0):
...
def contract_summary(args: Args) -> str:
    return "\n".join(
        [
            "ROS 2 EEF deployment contract:",
            "- policy state/action = 20D [left_eef9d,left_gripper,right_eef9d,right_gripper]",
...
def _build_environment(args: Args) -> Ros2DualEnvironment:
    ...
    return Ros2DualEnvironment(
        ...
        dry_run=args.dry_run,
        publish_actions=args.publish_actions,
        observation_adapter=EefObservationAdapter(task=args.prompt),
        action_adapter=EefActionAdapter(),
        prompt=args.prompt,
...
def main(args: Args | argparse.Namespace | None = None) -> int:
    ...
    try:
        print("\nInitializing EEF ROS 2 environment...")
        environment = _build_environment(args)
        print("EEF ROS 2 environment initialized")
