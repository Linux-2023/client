...
do
    ...
    # 执行命令a
    echo "执行pour（reset）"
    python examples/piper/main.py --args.prompt "Pour the objects in the box onto the table" --args.host "0.0.0.0" --args.high_camera_id 8 --args.left_wrist_camera_id 4 --args.port 9000
    if [ $? -ne 0 ]; then
        echo "警告：pour执行时出现错误"
    fi
    ...
    # 执行命令b
    echo "执行pick up"
    python examples/piper/main.py --args.prompt "pick up anything and put them in the box" --args.host "0.0.0.0" --args.high_camera_id 8 --args.left_wrist_camera_id 4 --args.record_mode
    if [ $? -ne 0 ]; then
