...
- Press 'ESC' to start recording an episode.
- Move the robot arm or interact with the environment.
- Press 'ESC' again to stop recording. The episode will be saved to a unique HDF5 file.
...
def display_observations(obs: dict, window_name: str = "Observations", recording: bool = False):
    ...
    for cam_name, img in images_dict.items():
        ...
        elif img.ndim == 3:  # Already HWC
            img_hwc = img
        else:
            print(f"Warning: Unexpected image shape {img.shape} for {cam_name}")
            continue
        
        # 确保是3通道BGR格式
        if img_hwc.shape[2] == 1:  # Grayscale
            img_bgr = cv2.cvtColor(img_hwc, cv2.COLOR_GRAY2BGR)
        elif img_hwc.shape[2] == 3:  # RGB to BGR
            img_bgr = img_hwc[:, :, ::-1].copy()
        else:
            print(f"Warning: Unexpected channel count {img_hwc.shape[2]} for {cam_name}")
            continue
        
        # 确保数据类型正确
        if img_bgr.dtype != np.uint8:
            img_bgr = np.clip(img_bgr * 255, 0, 255).astype(np.uint8) if img_bgr.max() <= 1.0 else img_bgr.astype(np.uint8)
        
        # Resize and center crop to 448x448 for better visibility
        target_size = 448
        h, w, _ = img_bgr.shape
        ...
        label_text = f"{cam_name} (REC)" if recording else cam_name
        # Adjusted font size and position for smaller image
        cv2.putText(img_cropped, label_text, (5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, label_color, 1)
        
        display_images.append(img_cropped)
    
    # 如果没有图像，创建一个占位符显示状态信息
    if not display_images:
        placeholder = np.zeros((224, 400, 3), dtype=np.uint8)
        state = obs.get('state', None)
        if state is not None:
            state_text = f"State: {len(state)} dims"
            cv2.putText(placeholder, state_text, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            cv2.putText(placeholder, "No cameras enabled", (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 1)
            if recording:
                cv2.putText(placeholder, "RECORDING", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)
...
def save_episode_to_hdf5(episode_data: list, directory: str = "recorded_data", prompt: str = ""):
    ...
    with h5py.File(filename, "w") as f:
        ...
        f.create_dataset("action", (num_steps,) + state_shape, dtype=first_obs['observation.state'].dtype)
        ...
        if 'task' in first_obs:
            ...
            print("No prompt found in observations; skipping task dataset.")
            prompt = "Pick up anything on the table and put it in the basket."
            f.create_dataset("task", (num_steps,), dtype=np.dtype(f'S{len(prompt)}'))
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="全局RealSense相机序列号（默认: None，使用第一个可用的RealSense相机）"
    )
...
def main():
    ...
    # 创建双臂环境
    print("=== 双臂 Piper 数据采集 ===")
    print(f"左臂CAN端口: {args.left_can_port}")
    print(f"右臂CAN端口: {args.right_can_port}")
    print(f"全局相机(RealSense)序列号: {args.high_camera_id if args.high_camera_id else '自动选择'}")
    print(f"左腕相机ID: {args.left_wrist_camera_id}")
    print(f"右腕相机ID: {args.right_wrist_camera_id}")
    ...
    try:
        ...
        env.reset()
        print("\nReady to record. Press ESC to start/stop recording, 'q' to quit.")
        ...
        while True:
            ...
            if key == 32:  # ESC key
                if not recording:
                    ...
                    print("\nStopping recording...")
                    recording = False
                    save_episode_to_hdf5(episode_data, directory="datasets_raw/Insert the straw into the cup", prompt=prompt)
                    print("Ready for next episode. Press ESC to start.")
            elif key == ord('q'):
                break
