...
class RealSenseCamera:
    ...
    - serial_number: RealSense camera serial number (optional, None for first available)
    - width, height, fps: stream config
    - format: pyrealsense2 format (default rs.format.bgr8)
    """

    def __init__(self, width: int = 640, height: int = 480, fps: int = 30,
                 fmt=rs.format.bgr8, serial_number: str = None):
        self.width = width
        self.height = height
        self.fps = fps
        self.fmt = fmt
...
class USBCamera:
    ...
    def start(self, timeout: float = 2.0):
        ...
        # Try to set MJPG format first to save bandwidth and achieve higher FPS
