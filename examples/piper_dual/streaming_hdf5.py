...
class StreamingEpisodeWriter:
    ...
    @classmethod
    def open(cls, path: Path, metadata: dict, jpeg_quality: int = 90, flush_every: int = 30) -> "StreamingEpisodeWriter":
        ...
        try:
            _initialize_file(h5_file, metadata, int(jpeg_quality), int(flush_every))
        except Exception:
            h5_file.close()
            partial_path.unlink(missing_ok=True)
            raise
        return cls(final_path=final_path, partial_path=partial_path, _file=h5_file, _flush_every=int(flush_every))
    ...
    def append(self, frame: SynchronizedFrame) -> None:
        ...
        sensor_timestamps = _coerce_sensor_mapping(frame.sensor_timestamps, "sensor_timestamps")
        sync_error = _coerce_sensor_mapping(frame.sync_error, "sync_error")
...
def _initialize_file(h5_file: h5py.File, metadata: dict, jpeg_quality: int, flush_every: int) -> None:
    ...
    h5_file.attrs["units_json"] = json.dumps(
        {"state": "mixed_rad_m", "action": "mixed_rad_m", "timestamp": "seconds", "sync_error": "seconds"},
        sort_keys=True,
        separators=(",", ":"),
    )
    ...
    h5_file.attrs["timing_counters_json"] = json.dumps(
        {"frames_written": 0, "flush_every": flush_every, "finalized": False}, sort_keys=True, separators=(",", ":")
    )
...
def _coerce_sensor_mapping(values: dict[str, float], field: str) -> dict[str, float]:
