...
class PiperEefRot6dInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        if state.shape[-1] != 20:
            raise ValueError(f"Expected a 20D EEF state, got shape {state.shape}")

        in_images = data["images"]
        unexpected = set(in_images) - set(self.EXPECTED_CAMERAS)
        if unexpected:
            raise ValueError(f"Unexpected EEF camera names: {tuple(sorted(unexpected))}")
        ...
        if "actions" in data:
            ...
            if actions.shape[-1] != 20:
                raise ValueError(f"Expected 20D EEF actions, got shape {actions.shape}")
...
@dataclasses.dataclass(frozen=True)
class PiperEefRot6dOutputs(transforms.DataTransformFn):
    def __call__(self, data: dict) -> dict:
