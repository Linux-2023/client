                ...
                return _transforms.Group(
                    inputs=[
                        ...
                        _transforms.TokenizePrompt(
                            ...
                            advantage_input=model_config.advantage_input
                        ),
                        _transforms.PadStatesAndActions(model_config.action_dim),
                        _transforms.PadStatesAndActions_Prev(model_config.action_dim),
                    ],
                )
...
class LeRobotFlexivDataConfig(DataConfigFactory):
    ...
    # If provided, will be injected into the input data if the "prompt" key is not present.
    default_prompt: str | None = None
    # If true, this will convert the joint and gripper values from the standard flexiv space to
    # the space used by the pi internal runtime which was used to train the base model. People who
    # use standard flexiv data should set this to true.
    ...
    @override
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        # Mutual exclusion: pose mode and delta-joint mode cannot be both enabled.
        ...
        data_transforms = _transforms.Group(
            inputs=[flexiv_policy.FlexivInputs(adapt_to_pi=self.adapt_to_pi)],
            outputs=[flexiv_policy.FlexivOutputs(adapt_to_pi=self.adapt_to_pi)],
        )
        if self.use_delta_joint_actions:
            # Flexiv: 16-dim state (left 7 joints + gripper, right 7 joints + gripper)
            # Delta on joints (0-6, 8-14), absolute on grippers (7, 15).
        ...
        if self.use_pose:
            # Pose mode: apply relative pose for both left/right arms (exclude grippers).
            delta_pose_mask = _transforms.make_bool_mask(7, -1, 7, -1)
            data_transforms = data_transforms.push(
                inputs=[_transforms.DeltaPose(delta_pose_mask)],
                outputs=[_transforms.AbsolutePose(delta_pose_mask)],
            )

        model_transforms = ModelTransformFactory(default_prompt=self.default_prompt)(model_config)

        def _update_repack_struct(group: _transforms.Group, updates: dict[str, str]) -> _transforms.Group:
            new_inputs = []
            for t in group.inputs:
                if isinstance(t, _transforms.RepackTransform):
                    struct = dict(t.structure)
                    struct.update(updates)
                    t = dataclasses.replace(t, structure=struct)
                new_inputs.append(t)
            return _transforms.Group(inputs=tuple(new_inputs), outputs=group.outputs)

        # Modify the existing repack structures directly instead of pushing new ones.
        computed_repack = self.repack_transforms
        repack_updates: dict[str, str] = {}
        if getattr(model_config, "advantage_input", False):
            repack_updates["advantage"] = "advantage"
        if self.use_pose:
            repack_updates["prev_state"] = "observation.prev_state"
        if repack_updates:
            computed_repack = _update_repack_struct(computed_repack, repack_updates)

        return dataclasses.replace(
            self.create_base_config(assets_dirs, model_config),
            repack_transforms=computed_repack,
            data_transforms=data_transforms,
            model_transforms=model_transforms,
            action_sequence_keys=self.action_sequence_keys,
        )
...
_CONFIGS = [
    ...
    TrainConfig(
        ...
        policy_metadata={"reset_pose": [8.84, -44.50, -13.61, -82.2, 86.35,8.2, 40.83, 0.0, -6.78, 46.79, 8.76, 67.47, 93.46, 2.01, -22.59, 0.0]},
    ),
    TrainConfig(
        name="pi05_piper",
        model=pi0_config.Pi0Config(pi05=True),
    ...
    TrainConfig(
        name="pi05_piper_dual_fold_towel_eef_xyz3d_100",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_dim=32,
            action_horizon=50,
    ...
    TrainConfig(
        name="pi05_piper_dual_block_drawer_eef_xyz3d_100",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_dim=32,
            action_horizon=50,
        ...
        data=LeRobotPiperEefXyz3dDataConfig(
            repo_id="HITdongdong/piper_dual_block_drawer_eef_xyz3d_100",
            base_config=DataConfig(prompt_from_task=True),
            default_prompt="Put the block in the drawer.",
        ),
        weight_loader=weight_loaders.CheckpointWeightLoader(
            "/pfs/pfs-7jnepv/lgd/.cache/openpi/openpi-assets/checkpoints/pi05_base/params"
        ),
    ...
    #
    TrainConfig(
        name="pi05_flexiv_train",
        model=pi0_config.Pi0Config(pi05=True, advantage_input=False),
        data=LeRobotFlexivDataConfig(
            #use_delta_joint_actions=False,
            repo_id="flexiv_lerobot_data_qpos",
            assets=AssetsConfig(
                assets_dir="./assets/pi05_flexiv_train",
                asset_id="flexiv_lerobot_data_qpos",
            ),
            base_config=DataConfig(prompt_from_task=True),
            repack_transforms=_transforms.Group(
                inputs=[
    ...
    TrainConfig(
        name="pi05_flexiv_pose_train",
        model=pi0_config.Pi0Config(pi05=True, advantage_input=False),
        data=LeRobotFlexivDataConfig(
            use_delta_joint_actions=False,
            use_pose=True,
            repo_id="flexiv_lerobot_data_pose",
            assets=AssetsConfig(
                assets_dir="./assets/pi05_flexiv_pose_train",
                asset_id='flexiv_lerobot_data_pose',
            ),
            base_config=DataConfig(prompt_from_task=True),
            repack_transforms=_transforms.Group(
                inputs=[
    ...
    TrainConfig(
        name="pi05_piper_pick_cube",
        model=pi0_config.Pi0Config(pi05=True, advantage_input=True),
        data=LeRobotPiperDataConfig(
            #use_delta_joint_actions=False,
            repo_id="piper_lerobot_data",
            assets=AssetsConfig(
                assets_dir="gs://openpi-assets/checkpoints/pi05_base/assets",
                asset_id="trossen",
            ),
