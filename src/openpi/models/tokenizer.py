...
class PaligemmaTokenizer:
    ...
    def tokenize(self, prompt: str, state: np.ndarray | None = None, advantage: str | None = None) -> tuple[np.ndarray, np.ndarray]:
        cleaned_text = prompt.strip().replace("_", " ").replace("\n", " ")
        if advantage is not None:
            cleaned_advantage = advantage.strip().replace("_", " ").replace("\n", " ")
            suffix_advantage = f", {cleaned_advantage}"
        else: 
            suffix_advantage = ""
        if state is not None:
            ...
            full_prompt = f"Task: {cleaned_text}, State: {state_str}{suffix_advantage};\nAction: "
            tokens = self._tokenizer.encode(full_prompt, add_bos=True)
        else:
            # This is the Pi0 format, where the state is part of the continuous action expert input.
            # tokenize "\n" separately as the "start of answer" token
            tokens = self._tokenizer.encode(cleaned_text+suffix_advantage, add_bos=True) + self._tokenizer.encode("\n")
