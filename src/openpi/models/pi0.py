    ...
    def sample_actions(
        ...
        *,
        ...
        def _maybe_scalar(value):
            if value is None:
                return None
            value_array = jnp.asarray(value).reshape(-1)[0]
            return jax.lax.convert_element_type(value_array, jnp.int32)

        obs_action_horizon = _maybe_scalar(observation.action_horizon)
        obs_actions_during_latency = _maybe_scalar(observation.actions_during_latency)
        if noise is None:
        ...
        def step(carry):
            ...
            if observation.prev_action is None: # no rtc
                _, v_t = denoise(x_t)
                #print("Without RTC")
            else: # rtc
                #print("With RTC!!!!!!!!!!!!!!!!!!!")
                ...
                prefix_attention_horizon = self.action_horizon - obs_action_horizon
                weights = get_prefix_weights(
                    obs_actions_during_latency, prefix_attention_horizon, self.action_horizon, "exp"
                )

                error = (observation.prev_action - x_1) * weights[None, :, None]

                # prev_action_slice = jax.lax.dynamic_slice(
                #     observation.prev_action,
                #     start_indices=(0, 0, 0),
                #     slice_sizes=(1, 8, 14)
                # )
                # debug.print("prev action {}", prev_action_slice[0])

                pinv_correction = vjp_fun(error)[0]
                # constants from paper
                inv_r2 = (tau**2 + (1 - tau) ** 2) / (1-tau ** 2)
                c = jnp.nan_to_num((1-tau) / tau, posinf=beta)
                guidance_weight = jnp.minimum(c * inv_r2, beta)
                v_t = v_t - pinv_correction * guidance_weight
        ...
        #     print(i, time)
        # x_0 = x_t
        if observation.prev_action is not None and obs_action_horizon is not None and obs_actions_during_latency is not None:
            prefix_attention_horizon = self.action_horizon - obs_action_horizon
            weights = get_prefix_weights(obs_actions_during_latency, prefix_attention_horizon, self.action_horizon, "zeros")
            error = (observation.prev_action - x_0) * weights[None, :, None]
            # Calculate mean squared error (MSE) and mean absolute error (MAE) as scalars
            mse = jnp.mean(jnp.square(error))
            mae = jnp.mean(jnp.abs(error))
            debug.print("mother fucker error MSE: {}, MAE: {}", mse, mae)
        return x_0
